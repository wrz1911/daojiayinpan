...
- 2026-09-21 **Windows 开发环境固化**(项目自 Linux `~/src/qimen` 迁至 Windows `D:\src\daojiayinpan` 后, 构建链的几处 Linux 假设逐项收口并落库):
  ...
  - **本地 debug 包与 CI release 包的差距**: debug = Android Debug 证书 + versionName 1.0/versionCode 1 + minSdk 23/targetSdk 35 + 无 R8(4.81MB); CI release = 正式 keystore + 1.4/10400 + minSdk 31/targetSdk 34 + R8 混淆与资源压缩 + 自绘图标(约 2.1MB)。**本地要打 release 必须有 `android/qimen-release.keystore` 与 `android/gradle.properties` 里的口令**(两者都不入库)
