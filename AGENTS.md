...
- 2026-09-21 **Windows 开发环境固化**(项目自 Linux `~/src/qimen` 迁至 Windows `D:\src\daojiayinpan` 后, 构建链的几处 Linux 假设逐项收口并落库):
  ...
  - **Android 构建链(可选)**: `npm run setup:android` 一键装 JDK 21(Adoptium Temurin 官方 zip) + Android SDK(commandline-tools / platform-tools / platforms;android-35 / build-tools;35.0.0)。**全程免管理员、无 UAC**: zip 解压到 `D:\dev\jdk-21` 与 `D:\Android\Sdk`, 环境变量只写用户级(JAVA_HOME / ANDROID_HOME / ANDROID_SDK_ROOT / PATH), 不碰系统目录。`sdkmanager` 的许可与组件安装由脚本喂 `y` 完成
