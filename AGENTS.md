...
- **⑭ 局数并回节气行(2026-09-22)**: 月将挪走后节气行只剩"节气区间 + [自选] + 阴遁X局", 实测**放得下**(含自选局场景也未换行), 故把 ⑫ 的两行结构收回一行, 节气行 43→24px。**信息表最终 131px, 滚动高 746px**(视口 805, 余 59px)。**回看这一段的三次反复很有代表性**: ⑫ 拆两行(因为节气+农历+月将+局数挤不下) → ⑬ 月将移走(它一月一换, 不该占局数行) → ⑭ 既然腾出位置就并回一行。**布局取舍要跟着"哪些信息必须同行"变化, 不是一次定死**(改前需滚动 157px)
