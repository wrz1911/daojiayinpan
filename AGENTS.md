...
- 五种盘型:1=时盘 2=刻盘 3=心盘 4=山向 5=穿壬;纯 HTML+JS 前端,Tauri 2 桌面 + Capacitor Android,当前版本 **1.4.1**(Android versionCode 10401; CI 的 APK versionName 取两段故显示 "1.4")
...
- **⑬ 月将挪到农历后、农历去括号去"日"(2026-09-22)**: 用户指出**月将变化慢**(一个月一换, 比局数稳定得多), 不该占着局数行的位置 → 移到日期行、跟农历并排: `日期 | 2026-09-22 23:31:00 八月十三【月将巳】`; 局数行随之只剩 `【自选】阴遁X局`。农历格式也按要求改为"八月十三"式: **去括号、去末尾"日"字**(`nongliShort = nongli.replace(/^\d+年/,'').replace(/日$/,'')`)。**注意 `window._nongliFull` 仍保留完整农历**(含"日"), 保存的排盘记录里时间串依旧是完整格式, 只有界面显示精简。实测: 表高 150px、滚动高 765px **均未变**(只是把月将从下一行挪上来, 不占额外空间)
- **⑭ 局数并回节气行(2026-09-22)**: 月将挪走后节气行只剩"节气区间 + [自选] + 阴遁X局", 实测**放得下**(含自选局场景也未换行), 故把 ⑫ 的两行结构收回一行, 节气行 43→24px。**信息表最终 131px, 滚动高 746px**(视口 805, 余 59px)。**回看这一段的三次反复很有代表性**: ⑫ 拆两行(因为节气+农历+月将+局数挤不下) → ⑬ 月将移走(它一月一换, 不该占局数行) → ⑭ 既然腾出位置就并回一行。**布局取舍要跟着"哪些信息必须同行"变化, 不是一次定死**
- **⑮ 四柱/五柱: 竖排 + 加大字号(2026-09-22 用户要求, 山向除外)**: "凡事牵扯到四柱五柱的内容一律竖排并加大字号, 不用刻意缩小"。此前为省 22px 把时盘/刻盘的干支改成**横排**(`丙午`), 方向错了 —— 已改回**竖排**(干支各一行, `<br>` 分隔)。做法: 新增专属 class **`sizhu-v`** 挂到四个盘型的四柱上(时/刻 `#nianzhu` 等 5 个 id、心盘两处循环生成、命理 `qimen_mingli.js`、穿壬 `qimen_chuanren.js`), CSS `#panHead #headTable .sizhu-v{font-size:var(--pan-sz-v)!important}`(手机 20px/24px, 默认 22px/26px, 宽屏 30px/34px)。**用专属 class 而不是直接改 `--pan-fs-sz`, 就是为了让山向例外** —— 山向的"干支"行虽也挂 `class="sizhu"`, 但不是四柱(它是单柱横排两格), **不加 `sizhu-v`** 故字号仍是 16px。**实测: 四柱 20px/高 53px/2 行**(原 16px/27px/1 行); 信息表 131→157px, 滚动高 772px(视口 805 仍一屏放下)(改前需滚动 157px)
...
- **玄女16诀加入心盘(2026-09-22 用户要求, 与金口诀并列)**: 原来心盘的 `#btnTable3` 只有金口诀(代码注释写着"地八神/人八神/玄女16诀是时盘专用, 不加"), 现按用户要求加入玄女16诀, 与时盘同序并列(`btnXuanNv` → `btnJinKou`); 地八神/人八神仍不加。**⚠️ 不能只加按钮**: `xuanNv16()` 依赖 `window._shiZhi` 与 `window._isYin`(阳顺阴逆), 而这两个值**原先只在 `renderPan` 里赋值**(726-727 行), **心盘从未设过** → 直接加按钮会取到 undefined、排不出盘。已在 `renderXinpan` 里从 `_xpBgSizhu` 的时柱(`sizhuParts[3]`)与 `_xpBgIsYin` 补齐。**教训: 把一个功能搬到新盘型时, 先 grep 它依赖的全局量在目标盘型的渲染路径里有没有被赋值**
- **⚠️ 删容器留下的死逻辑: 自选局曾"平白无故丢失"(2026-09-22 用户报, 已修)**: ⑧ 删除空的 `#zxjRow` 时, 我漏看了 `setPanType` 里那段显隐代码是**包在 `if(zxjRow)` 里**的 —— 容器一删, `getElementById('zxjRow')` 恒为 null, **整个分支永不执行**, `#zxjSpan` 的显隐彻底失效。而 `doChuanRen`/`doMingli` **会主动把 `#zxjSpan` 设为 none**, 于是"进过穿壬/命理 → 切回时盘/刻盘"时**没人把它恢复显示**, 用户看到的就是自选局不见了。修法: 显隐逻辑直接基于 `#zxjSpan` 自身判断, 不再依赖任何外层容器: `zs.style.display=(t===3||t===4||t===5||t===6)?'none':'flex'`。**教训: 删元素前先 grep 它被哪些 `if(元素)` 守卫的逻辑引用 —— 守卫会静默地把整块逻辑变成死代码, 不报错也不进日志。** 另: 心盘(3) 也在隐藏之列 —— **心盘是用户自定义局, 本就不该有自选局**(用户明确); 只有时盘/刻盘显示自选局
- **⚠️ GitHub Pages 必须手动启用一次(2026-09-22)**: README 加"在线体验"链接时发现 `https://wrz1911.github.io/daojiayinpan/` 直接 **404** —— 排查发现 `gh-pages` 分支**早就由 CI 推上去了**(`peaceiris/actions-gh-pages@v4`, 见 release.yml 的 Prepare Pages / Deploy to GitHub Pages 步骤), 但**该 action 只负责推分支, 不会启用 Pages 站点**(仓库 `has_pages=false`)。启用命令: `gh api -X POST repos/wrz1911/daojiayinpan/pages -f 'source[branch]=gh-pages' -f 'source[path]=/'`(得到 build_type=legacy / https_enforced=true)。**注意: `gh api repos/.../pages` 在未启用时返回 404 而不是空对象**, 别把它当成"仓库不存在"。启用后首次构建约 1 分钟; 实测根页 200 并重定向到 `qimen_app/yinpan.html`, 排盘页 4430 字节, 线上 bundle 版本 **v1.4.1**。**此设置是仓库级、一次性的, 不会随 CI 重复部署而丢失**
