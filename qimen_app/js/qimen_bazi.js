/* 八字排盘 —— 照搬热卜 mod=bazi 结果页(传统模式)
 *
 * 入口: 命理模块 #btnTable1 「长生状态」右边新增「八字排盘」按钮
 * 计算: tyme4ts (window.tyme), 渲染结构/配色照搬热卜 baziPan 页面
 *
 * 与热卜的字段对应(已用服务端真实结果逐项核对):
...
(function () {
  ...
  /* 天干五行色: 木绿 火红 土棕 金橙 水蓝 (热卜 GanColor 原值) */
  ...
  /* 六十年纳音用字: 热卜写"霹雷火/沙中金", tyme 写"霹雳火/砂中金", 统一成热卜用字 */
  ...
  function baziChart(o) {
    ...
    /* 早晚子时: 默认晚子时日柱算次日; 开启后算当天(与热卜 ziType=1 一致)。
    ...
    pillars.forEach(function (p, i) {
      ...
      p.sha = SHA_ORDER.filter(function (n) { return got[n]; });   // 输出顺序照热卜
    });
    ...
    /* 节气文本: 出生时刻在相邻两个节气之间的位置(格式照热卜, 数值按实际算) */
